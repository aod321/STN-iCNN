...
from celebAMask_preprocess import Resize, OrigPad, ToPILImage
from torch.utils.data import DataLoader
# from dataset import HelenDataset
from dataset import CelebAMask
from model import SelectNet, SelectNet_resnet
import torchvision
from helper_funcs import calc_centroid, affine_crop, affine_mapback
...
parser.add_argument("--pretrainA", default=1, type=int, help="Load pretrainA")
...
parser.add_argument("--cuda", default=4, type=int, help="Choose GPU with cuda number")
...
class TrainModel(TemplateModel):
    ...
    def train_loss(self, batch):
        ...
        for i in range(5):
            theta_label[:, i, 0, 0] = (127. - 1.) / (W - 1)
            theta_label[:, i, 0, 2] = -1. + (2. * points[:, i, 1]) / (W - 1)
            theta_label[:, i, 1, 1] = (127. - 1.) / (H - 1)
            theta_label[:, i, 1, 2] = -1. + (2. * points[:, i, 0]) / (H - 1)

        theta_label[:, 5, 0, 0] = (255. - 1.) / (W - 1)
        theta_label[:, 5, 0, 2] = -1. + (2. * points[:, 5, 1]) / (W - 1)
        theta_label[:, 5, 1, 1] = (255. - 1.) / (H - 1)
        theta_label[:, 5, 1, 2] = -1. + (2. * points[:, 5, 0]) / (H - 1)
    ...
    def eval_error(self):
        ...
        for batch in tqdm(self.eval_loader):
            ...
            assert orig_label.shape == (N, 9, 512, 512)
            cens = torch.floor(calc_centroid(orig_label))
        ...
        test = theta[:, 5]
        grid = F.affine_grid(theta=test, size=[N, 3, 255, 255], align_corners=True)
        temp.append(F.grid_sample(input=orig, grid=grid, align_corners=True))
    ...
    def load_pretrained(self, model, mode=None):
        ...
        path_modelA = os.path.join("/home/yinzi/data3/new_train/exp_A/f42800da", 'best.pth.tar')
        if mode == 0:
            path_modelB_select_net = os.path.join("/home/yinzi/data3/new_train/checkpoints_B_selectnet/cab2d814",
                                                  'best.pth.tar')
        elif mode == 1:
            path_modelB_select_net = os.path.join("/home/yinzi/data3/new_train/checkpoints_B_resnet/1a243c7e",
                                                  'best.pth.tar')
